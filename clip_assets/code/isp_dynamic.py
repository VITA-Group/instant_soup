...
def all_dicts(mask_dicts):
    merged_mask = {}
    model_keys = mask_dicts[0].keys()
    for key in model_keys:
        merged_mask[key] = mask_dicts[0][key]
        for i in range(1, 5):
            merged_mask[key] += mask_dicts[i][key]
        merged_mask[key] = (merged_mask[key] == 5).type(torch.float32)
    del mask_dicts
    return merged_mask
...
def dynamic_merge(mask_dicts):
    merged_mask = {}
    model_keys = mask_dicts[0].keys()
    for key in model_keys:
        merged_mask[key] = copy.deepcopy(mask_dicts[0][key])
        for i in range(1, len(mask_dicts)):
            merged_mask[key] += mask_dicts[i][key]
        merged_mask[key] = torch.clip(merged_mask[key], 0, 1)
    del mask_dicts
    return merged_mask
...
def main():
    ...
    fopen = open("logs/ISP_Mergedict_dynamic_1000.txt", "a")
    ...
    iter_training_ratio = 0.075

    
    fopen.write(f"------------ Target Sparsity : {args.target_sparsity} || Dataset : {args.train_dataset}-------------\n")
    total_ratio_trained = 0.0
    for i in range(0, epoch_count):
        
        print("------------------------Epoch {}--------------------".format(i))
        ratio_trained = iter_training_ratio * (i + 1)
        total_ratio_trained += ratio_trained
        main_trainer.train_epoch(ratio_trained, args.seed)
        fopen.write("Main Trainer {} sparsity : {:.4} % and performance : {} and trained : {} \n".format(i, main_trainer.pruner.get_sparsity_ratio(), main_trainer.evaluate_model(), ratio_trained))
        ...
        mask_dicts = {}
        for j in range(0, args.denoiser):
            ...
            del aux_trainer
        merged_mask = dynamic_merge(mask_dicts)
        main_trainer.pruner.prune_model_custom(merged_mask)
        fopen.flush()

    epoch_left = epoch_count - int(total_ratio_trained)
